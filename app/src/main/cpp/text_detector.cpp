...
namespace ppocrv5 {
    ...
    namespace {
        ...
        constexpr float kBinaryThreshold = 0.1f;  // 进一步降低二值化阈值，最大化召回率
        constexpr float kBoxThreshold = 0.4f;  // 提高置信度阈值，提高准确率
        constexpr float kMinBoxArea = 10.0f;  // 提高最小区域面积阈值，过滤过小的文本框
        constexpr float kUnclipRatio = 1.6f;  // 降低unclip比例，使检测框更贴合
