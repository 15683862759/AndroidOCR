...
namespace ppocrv5::image_utils {
    ...
    // Stable Letterbox Resize Implementation
    void ResizeLetterbox(const uint8_t *src, int src_w, int src_h, int src_stride,
        ...
        int nw = static_cast<int>(src_w * scale);
        int nh = static_cast<int>(src_h * scale);
        pad_w = (dst_w - nw) / 2;
        pad_h = (dst_h - nh) / 2;

        std::memset(dst, 0, dst_w * dst_h * 4);

        float inv_scale = 1.0f / scale;

        for (int y = 0; y < nh; ++y) {
            float src_y = Clamp((y + 0.5f) * inv_scale - 0.5f, 0.f, src_h - 1.f);
            int y0 = static_cast<int>(src_y);
            int y1 = std::min(y0 + 1, src_h - 1);
            float dy = src_y - y0;
            ...
            const uint8_t *row0 = src + y0 * src_stride;
            const uint8_t *row1 = src + y1 * src_stride;
            uint8_t *dst_row = dst + (y + pad_h) * dst_w * 4 + pad_w * 4;

            for (int x = 0; x < nw; ++x) {
                float src_x = Clamp((x + 0.5f) * inv_scale - 0.5f, 0.f, src_w - 1.f);
                int x0 = static_cast<int>(src_x);
                int x1 = std::min(x0 + 1, src_w - 1);
                float dx = src_x - x0;
